  ...
  return (
    <PageContainer>
      ...
      {!loading && !error && purchase && (
        submitting ? <div className="p-8 text-center text-sm text-[var(--text-muted)] animate-pulse">Saving purchase transaction... Please wait.</div> :
          <PurchaseForm initialData={purchase} onSave={handleSave} onCancel={() => navigate(`/inventory/purchases/${id}`)} companyState={activeCompany ? activeCompany.state : ''} />
      )}
    </PageContainer>
  );
}
